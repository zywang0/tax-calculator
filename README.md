...
- `tax-calculator-domain/src/main/java/org/study/domain/tax/model/entity`
- `tax-calculator-domain/src/main/java/org/study/domain/tax/model/valobj`
- `tax-calculator-domain/src/main/java/org/study/domain/tax/service`
- `tax-calculator-trigger/src/main/java/org/study/trigger/http`
...
当前已完成 P0 最小可用版本，不包含数据库持久化。

已实现：

- 内存版规则仓储：`InMemoryTaxRuleRepository`。
- 默认规则版本：`CN_IIT_2026`。
- 领域计算服务：`IitCalculateService`、`IitCalculateServiceImpl`。
- 单月工资个税试算接口：`POST /api/tax/iit/salary/monthly/calculate`。
- 规则详情查询接口：`GET /api/tax/iit/rules/{ruleVersion}`。
- 基础参数校验：年份、月份、金额非负、规则版本存在。
- 单元测试：覆盖 0 税额、档位边界、跨档、累计已缴税、非法月份、负数金额、未知规则版本。
